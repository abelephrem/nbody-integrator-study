...
At the end of Stage 8, Leapfrog is formally selected as the ground-truth generator for GNN training data, justified by the Q2, Q3 and Q4 results.

The justification needs stating precisely, because the obvious version of it is wrong. Trajectories are stored with exact Newtonian accelerations alongside positions and velocities, so the supervised targets for G1 are correct regardless of which integrator produced the states. The choice of integrator does not corrupt the labels; it determines **which states get visited**. That is where the argument actually lives:

- **Directional bias in the training distribution.** Q3 and Q4 together show that RK4's long-run error is not noise but a coherent inward spiral at near-constant eccentricity — energy and angular momentum shrink in the fixed ratio |ΔE/E| = 2|ΔL/L| predicted by Kepler geometry, confirmed to three significant figures. A dataset generated by RK4 is therefore a dataset in which every orbit is systematically contracting. A model with enough capacity to fit that data will reproduce the contraction. Leapfrog's energy error is bounded and oscillatory (Q2: q = 0.02) with no preferred direction, so there is no comparable artefact to learn.
- **Validity of the reference for G2 and G3.** Rollout error is measured *against* the ground-truth trajectory. A reference whose own error grows linearly and without bound over the long rollouts G2 requires would contaminate the measurement with its own drift, and that drift would be misattributed to the GNN. A bounded-error reference does not have this problem.
- **Symmetry preservation.** Leapfrog conserves angular momentum to round-off (Q3, ~1e-14), so the training distribution respects rotational symmetry exactly. Relevant to G3 and to the G4 generalisation test.
- **Cost.** One force evaluation per step against RK4's four, at four times the step count for equal accuracy on the eccentric case — roughly cost-neutral, and simpler to instrument for G5.

**What Leapfrog does not win on, and should not be claimed to.** On the eccentric orbit at 400 steps/orbit, RK4's energy error over 300 orbits is *smaller in magnitude* than Leapfrog's (8.75e-6 against 6.71e-4), and remains smaller at matched computational cost. RK4 is also the more accurate method for short-time position error, as Q1 confirms. The case for Leapfrog rests on the **character** of its error — bounded rather than secular, unbiased rather than directional, with exact angular momentum conservation — not on its magnitude. Stating this explicitly is what makes Q4 a finding rather than a benchmark table.
...
- **Bridge**: at the end of Stage 8, Leapfrog is formally confirmed as the ground-truth generator for GNN training data, justified by the Q2, Q3 and Q4 results (see the Part 1 bridge — the argument is about error *character*, not magnitude; RK4's energy error is smaller on the eccentric case). The data generation pipeline from Stage 5 is run using Leapfrog only, producing the final training dataset.
